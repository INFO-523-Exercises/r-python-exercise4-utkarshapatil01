...
Here we created two vector variables x and y, with linear relationship in them and added the noise to data.
...
The variable y_pred_test will contain the predicted values for the 'y' (target variable) based on the features in the test set.
...
In above scatter plot, x-values represent the true values for the y_test and y-values represent the predicted values for the closing price y_pred_test.
...
The Root mean squared error is a measure of the average deviation of predicted values from true values, and a lower RMSE generally indicates better model performance. In this case, an RMSE of 1.027 suggests that, on average, the model's predictions are approximately 1.027 units away from the true values.
...
Here we have negative slope shows the negative correlation, meaning that as the independent variable increases, the dependent variable tends to decrease.

```{r}
cat("Intercept =", intercept, "\n")
```

The intercept in a linear regression model represents the predicted value i.e. 0.97 of the dependent variable when all independent variables are set to zero.
...
Here high R-sqaured value indicates the good fit and suggests that the model does a really good job of predicting the value.
...
1.  X and X2 has strong positive correlation in them.

2.  X2 and X3 has strong positive correlation in them.

3.  X3 and X4 has strong positive correlation in them.

4.  X4 and X5 have a weak negative linear relationship between them, the negative sign indicates a negative correlation.
...
According to above plot, higher test error which suggests that model is not a good fit and may have overfitting issues. We can also see that train error goes on decreasing as the complexity of the model increases.
...
Here we can see that Sum of Absolute Weights has reduced to 0.75 only, indicating that Lasso has reduced the complexity of the model by shrinking some coefficients to exactly zero.
...
Here we can see that Sum of Absolute Weights has reduced to 0.75.
